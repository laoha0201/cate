...
namespace Laoha\Cate\Models\Traits;
...
// 重写了saveOrder方法 ,表中添加parents字段（上级列表）
trait ModelTree
{
    use SortableTrait;
     ...
     * @var string
     */
    protected $parentsColumn = 'parents';

    /**
     * @return string
     */
    public function getParentsColumn()
    {
     ...
     * @param string $column
     */
    public function setParentsColumn($column)
    {
        $this->parentsColumn = $column;
    }
     ...
     */
    public static function saveOrder($tree = [], $parentId = 0, $parents = '')
    {
        ...
        foreach ($tree as $branch) {
            ...
            if (empty($parentId)) {
                $node->{$node->getParentsColumn()} = '';
            } else {
                $node->{$node->getParentsColumn()} = empty($parents) ? $parentId :  $parents . ',' . $parentId;
            }

            $node->save();

            if (isset($branch['children'])) {
                static::saveOrder($branch['children'], $branch['id'], $node->{$node->getParentsColumn()});
            }
        }
    }
    ...
    protected static function boot()
    {
        ...
        static::saving(function (Model $branch) {
            ...
            $parentsColumn = $branch->getParentsColumn();
            ...
            if (!empty(Request::input($parentColumn))) {
                $pid = (new $branch())->where('id', Request::input($parentColumn))->first();
                if (empty($pid->$parentsColumn)) {
                    $branch->$parentsColumn = Request::input($parentColumn);
                } else {
                    $branch->$parentsColumn = $pid->$parentsColumn . ',' . Request::input($parentColumn);
                }
            }
            return $branch;
        });
